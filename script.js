// ===== Chart fallback (กันพังถ้า CDN ล่ม) =====
...
scales: { x: { ticks:{color:'#fff'}, grid:{color:'rgba(255,255,255,0.05)'} }, y:{ ticks:{color:'#fff'}, grid:{color:'rgba(255,255,255,0.04)'} } },
...
const MAX = 8; const base = active.length<=MAX?active:active.slice(0,MAX);
...
pieChart.data.labels = labels; pieChart.data.datasets[0].data = values; pieChart.data.datasets[0].backgroundColor = generateColors(values.length);
pieChart.update();
}
...
function init(){ applyTheme(themeSelect.value||'Dark'); for(let i=0;i<3;i++){ expenses.push(createExpenseRow()); } renderList(); updateCharts(); calcRemaining(); }
