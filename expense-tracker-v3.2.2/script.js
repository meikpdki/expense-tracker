...
(function(){ if (typeof window.Chart === "undefined") { window.Chart = function(){ return { data:{labels:[],datasets:[{data:[]}]}, update(){}, destroy(){} }; }; }})();
...
const lineChart = new Chart(lineCtx, { type: "line", data: { labels: [], datasets: [{ label: "Expenses", data: [], borderColor: "#23d3ff", backgroundColor: "rgba(35,211,255,0.4)", tension: 0.5, fill: true, pointRadius: 3 }] }, options: { responsive: true, maintainAspectRatio: false, scales: { x: { ticks: { color: "#fff" }, grid: { color: "rgba(255,255,255,0.05)" } }, y: { ticks: { color: "#fff" }, grid: { color: "rgba(255,255,255,0.04)" } } }, plugins: { legend: { labels: { color: "#fff" } } } });
const pieChart = new Chart(pieCtx, { type: "pie", data: { labels: [], datasets: [{ data: [], backgroundColor: [] }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false }, tooltip: { enabled: true } } });
...
function renderList(){
  ...
  expenses.filter(it=>it.name.toLowerCase().includes(keyword)).forEach((item)=>{
    ...
    const chooseBtn = document.createElement("button"); chooseBtn.className = "primary action-btn"; chooseBtn.textContent = "เลือกไฟล์"; chooseBtn.addEventListener("click", ()=>{ currentFileTargetId = item.id; hiddenFileInput.click(); });
    const viewBtn = document.createElement("button"); viewBtn.className = "primary secondary action-btn"; viewBtn.textContent = "view"; viewBtn.addEventListener("click", ()=>{ if(item.image){ previewImg.src = item.image; imgModal.style.display = "flex"; } else { alert("ยังไม่มีรูปในแถวนี้"); } });
    const delBtn = document.createElement("button"); delBtn.className = "danger action-btn"; delBtn.textContent = "Delete"; delBtn.addEventListener("click", ()=>{ expenses = expenses.filter(x=>x.id !== item.id); renderList(); updateCharts(); calcRemaining(); });
    row.append(dateInput, nameInput, amountInput, chooseBtn, viewBtn, delBtn); listEl.appendChild(row);
  });
}
...
function escapeHtml(s){ var map = { '&':'&amp;', '<':'&lt;', '>':'&gt;', '"':'&quot;', "'":'&#39;' }; return String(s).replace(/[&<>"']/g, function(m){ return map[m]; }); }
